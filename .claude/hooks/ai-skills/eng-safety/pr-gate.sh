...
#           MCP update_pull_request with draft: false, gh pr create --web   -> ask when a human
#           can answer (permission_mode default, acceptEdits, plan, auto); deny in every
#           other mode (bypassPermissions, dontAsk, missing) because nobody can confirm there
...
# Ask only when someone can answer the prompt; otherwise fail closed.
ready_policy() {
  case "$MODE" in
