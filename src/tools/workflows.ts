...
const STEP_SPEC_DESCRIPTION =
  ...
  "runWhen: \"true\"|\"false\" only run on that outcome\n" +
  ...
  "- { do: \"ai_chat\", prompt (required), title?, saveToFields?: [field names or ids] (max 10 — the answer is " +
  "extracted into these fields), model? }\n" +
      ...
      try {
        ...
        const buildTrigger = async (spec: Dict, allowWebhook: boolean): Promise<Dict> => {
          ...
          if (on === "inbound_webhook") {
            ...
            const t: Dict = { type: "folders", triggerSlug: "inbound_webhook" };
          ...
          if (on === "field_updated") {
            ...
            const t: Dict = { type: "folders", triggerSlug: "field_updated", values };
        ...
        const buildRules = async (rules: Array<Dict>): Promise<Dict[]> => {
          const out: Dict[] = [];
          for (const r of rules) {
            let field = String(r.field ?? "");
            // Custom-field filter rules use field ids; resolve names, but leave
            // canonical fields and webhook payload paths (dotted) untouched.
            if (!CANONICAL_FILTER_FIELDS.has(field) && !field.includes(".") && !isWebhookAutomation) {
              field = resolveField(field, await getFields());
            }
            const rule: Dict = { field, op: String(r.op ?? "eq") };
        ...
        for (let i = 0; i < steps.length; i++) {
          ...
          if (doType === "filter" || doType === "branch") {
            ...
            const block = {
              ...
              rules: await buildRules((spec.rules as Array<Dict>) ?? []),
            };
