...
```json
{
  "mcpServers": {
    "speakai": {
      "command": "npx",
      "args": ["-y", "@speakai/mcp-server@1.24.0"],
      "env": {
...
```sh
SPEAK_API_KEY=speak_sk_example_000000000000 npx @speakai/mcp-server@1.24.0 config test
```
...
4. Run `npx @speakai/mcp-server@1.24.0 config test` to validate the key and reach the API.
