...
**119 tools, 5 resources, and 3 prompts** over one workspace of recordings, transcripts,
...
```json
{
  "mcpServers": {
    "speakai": {
      "command": "npx",
      "args": ["-y", "@speakai/mcp-server@1.26.0"],
      "env": {
...
```sh
SPEAK_API_KEY=speak_sk_example_000000000000 npx @speakai/mcp-server@1.26.0 config test
```

## What the 119 tools cover
...
4. Run `npx @speakai/mcp-server@1.26.0 config test` to validate the key and reach the API.
